...
3. Выбрать токен → сертификат → **`F5`** → выбрать, куда скопировать
   (съёмные диски · папка рядом · реестр · папка КриптоПро).

Журнал операций по умолчанию **не ведётся** — программу носят на флешке и
запускают на чужих машинах, след с ИНН и отпечатками там не нужен. Чтобы вести
журнал рядом с программой, запустите с ключом:
